...
graficar_media_rango <- function(
  ...
  if (is.na(ref_label) || !nzchar(trimws(ref_label))) ref_label <- "Promedio general"
  ...
  if (isTRUE(mostrar_media) && identical(modo, "score_ref")) {
    ...
    bubble_text_size <- max(5.2, size_media * 1.55)
    ...
    if (identical(marker_style, "rectangulo")) {
      p_core <- p_core +
        ggplot2::geom_label(
          ...
          fill = sum_df$chip_fill,
          text.colour = chip_texto_color,
          border.colour = "#173B63",
          fontface = "bold",
          size = max(3.2, size_media * 1.20),
          linewidth = 0.30,
  ...
  if (isTRUE(mostrar_media) && identical(modo, "delta")) {
    ...
    if (identical(marker_style, "burbuja") || identical(marker_style, "rectangulo")) {
      p_core <- p_core +
        ggplot2::geom_label(
          ggplot2::aes(label = .data$marker_label, fill = .data$categoria),
          inherit.aes = TRUE,
          text.colour = chip_texto_color,
          border.colour = color_media,
          fontface = "bold",
          size = max(2.6, size_media),
          linewidth = 0.30,
